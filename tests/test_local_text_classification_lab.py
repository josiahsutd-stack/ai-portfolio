...
from local_text_classification_lab import (
    SourceMessage,
    build_subset,
    default_examples,
    predict_label,
...
def test_checked_in_public_subset_matches_provenance_manifest() -> None:
    project = Path("experiments/local-text-classification-lab/sample_data")
...
def test_public_subset_validator_detects_manifest_and_tsv_tampering(tmp_path: Path) -> None:
    source = Path("experiments/local-text-classification-lab/sample_data")
...
def test_real_model_artifacts_are_written(tmp_path: Path) -> None:
    result = write_training_artifacts(
        "experiments/local-text-classification-lab/sample_data/training_examples.json",
        tmp_path,
    )
...
def test_public_dataset_training_uses_held_out_test_set(tmp_path: Path) -> None:
    dataset_path = Path("experiments/local-text-classification-lab/sample_data/uci_sms_subset.tsv")
