...
```bash
streamlit run experiments/local-text-classification-lab/app.py
```

Generate model artifacts:

```bash
python experiments/local-text-classification-lab/evaluate_model.py
```
...
- Fitted model artifacts generated under `.artifacts/local-text-classification-lab/` during local evaluation. Runtime binaries are ignored by Git; metrics and reports remain versioned in `demo_outputs/`.
...
```bash
python experiments/local-text-classification-lab/scripts/build_uci_sms_subset.py
...
```bash
python -m pytest tests/test_local_text_classification_lab.py
```
