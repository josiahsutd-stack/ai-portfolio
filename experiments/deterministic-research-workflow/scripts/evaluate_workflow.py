...
from deterministic_research_workflow import ResearchWorkflow  # noqa: E402
...
def _write_report(summary: dict[str, object], output_path: Path) -> None:
    lines = [
        ...
        "",
        "Synthetic local evaluation for the Deterministic Research Workflow.",
        "",
        "## Summary",
        "",
...
def main() -> None:
    ...
    trace_db = output_dir / "workflow_eval_traces.sqlite"
    runtime_dir = REPO_ROOT / ".artifacts" / "deterministic-research-workflow"
    runtime_dir.mkdir(parents=True, exist_ok=True)
    memory_path = runtime_dir / "workflow_memory.json"
    if trace_db.exists():
        trace_db.unlink()
    ...
    traces = [
        ResearchWorkflow(docs_dir, memory_path=memory_path, trace_db_path=trace_db).run(task)
        for task in tasks
    ]
    denied_trace = ResearchWorkflow(
        docs_dir,
    ...
    (output_dir / "workflow_eval_summary.json").write_text(
        json.dumps(summary, indent=2) + "\n",
        encoding="utf-8",
    )
    _write_report(summary, output_dir / "workflow_eval_report.md")
    (output_dir / "sample_trace.json").write_text(
        json.dumps(_stable_trace(traces[0]), indent=2) + "\n",
        encoding="utf-8",
    )
    print(json.dumps(summary, indent=2))
    print(f"Wrote workflow demo outputs to {output_dir}")
