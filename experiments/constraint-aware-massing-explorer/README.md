...
streamlit run experiments/constraint-aware-massing-explorer/app.py
```

Optional local API:

```bash
python -m uvicorn constraint_aware_massing_explorer.api:app --app-dir experiments/constraint-aware-massing-explorer/src --reload
```
...
```mermaid
flowchart LR
  A["Synthetic or user-supplied numeric constraints"] --> B["Validated site scenario"]
  B --> C["Seeded typology generator"]
  C --> D["Hard-constraint validator"]
  D --> E["Geometric proxy objectives"]
  E --> F["Pareto front and weighted ranking"]
  F --> G["Option table, SVG plan, and isometric view"]
