...
def run_benchmark(
    ...
    summary = {
        ...
        "evaluated_candidates_per_mode": candidate_count * len(runs),
        "seeds": list(seeds),
        "run_count": len(runs),
        "aggregate": {
            ...
            "constraint_aware_mean_best_gfa_error_percent": round(
                mean(item["best_gfa_error_fraction"] for item in constrained) * 100,
                2,
            ),
            "baseline_mean_best_gfa_error_percent": round(
                mean(item["best_gfa_error_fraction"] for item in baseline) * 100,
                2,
            ),
        },
        "runs": runs,
