...
The document metric counts any chunk from the expected source. The exact-target metric requires one of the manually labeled supporting chunks. The page metric requires the expected source and one of the labeled pages. All 21 answerable cases have exact targets; 18 also have page targets.

| Granularity | n | Hit@1 | Hit@3 | MRR |
| --- | ---: | ---: | ---: | ---: |
| Expected document | 21 | `0.952` | `1.000` | `0.976` |
| Manually labeled exact chunk | 21 | `0.810` | `0.952` | `0.881` |
...
| Exact evidence target Hit@1 | `0.810` | `[0.600, 0.923]` | `21` manually labeled targets | Wilson score |
| Exact evidence target MRR | `0.881` | `[0.762, 0.976]` | `21` manually labeled targets | Percentile bootstrap |
...
Exact-target uncertainty: Hit@1 `0.810` has a 95% interval of `[0.600, 0.923]`, and exact-target MRR `0.881` has a 95% interval of `[0.762, 0.976]` across `21` manually labeled targets.
...
- Whether a correct document result also contains a manually labeled target chunk/page, reported as a separate metric.
...
- Exact-target Hit@1 is `0.810`, below document Hit@1 `0.952`; the 21 chunk targets and 18 page targets were manually selected from the fixed snapshot.
