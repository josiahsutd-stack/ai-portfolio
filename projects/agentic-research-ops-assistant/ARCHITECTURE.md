...
| --- | --- | --- |
| Planner | `agent.py` | Chooses a deterministic tool plan based on the task and allowed tools. |
| Tool registry | `agent.py` | Describes available tools and approval requirements. |
| Executor | `agent.py` | Runs tools, retries failures, records status, attempts, and latency. |
| Memory | `agent.py` | Writes the latest JSON trace to local memory. |
| Trace store | `trace_store.py` | Persists traces and trace evaluations to SQLite. |
| Reporter | `agent.py` | Builds a cited markdown report from retrieved local evidence. |
| Approval gate | `agent.py` | Marks the run as requiring human approval before external use. |

## Trace Contract

Every run produces an `AgentTrace` with:
