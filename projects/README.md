...
| Primary | [Construction Embodied Agent Simulator](vla-embodied-agent-simulator/README.md) | Engineered, semantic-raster, egocentric-state, and rendered-RGB imitation with a held-out appearance shift, 96 disjoint holdout scenarios, raw/filtered rollouts, failure traces, and a 12-scenario planar MuJoCo contact replay. | RGB pixels come from simulator state and filters see full rules; physics uses a planar body and static proxies, not physical-camera perception, mobile-robot dynamics, ROS, hardware, or safety validation. |
